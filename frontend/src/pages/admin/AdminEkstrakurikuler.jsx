...
export default function AdminEkstrakurikuler() {
  ...
  async function handleSubmit(e) {
    ...
    setStatus('Menyimpan...');
    try {
      if (editingId) await api.put(`/ekstrakurikuler/${editingId}`, form);
      else await api.post('/ekstrakurikuler', form);
      resetForm();
      load();
      setStatus('Tersimpan.');
    } catch (err) {
  ...
  async function handleDelete(id) {
    if (!confirm('Hapus ekstrakurikuler ini beserta seluruh galerinya?')) return;
    await api.del(`/ekstrakurikuler/${id}`);
  ...
  function openGaleri(id) {
    setManagingId(id);
    api.get(`/ekstrakurikuler/${id}`).then(setDetail).catch(() => {});
  }

  async function addGaleriFoto() {
    if (!galeriUrl) return;
    await api.post(`/ekstrakurikuler/${managingId}/galeri`, { url: galeriUrl, caption: galeriCaption });
    setGaleriUrl('');
    setGaleriCaption('');
  ...
  async function deleteGaleriFoto(galeriId) {
    ...
    await api.del(`/ekstrakurikuler/galeri/${galeriId}`);
    openGaleri(managingId);
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="admin-form">
        <div className="field"><label>Nama Ekstrakurikuler</label>
      ...
      {managingId && detail && (
        <section className="admin-galeri-manager">
          <h2>Galeri Foto Kegiatan: {detail.nama}</h2>
